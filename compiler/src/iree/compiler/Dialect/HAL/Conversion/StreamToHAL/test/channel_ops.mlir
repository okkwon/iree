...
func.func @channel_create() -> !stream.channel {
  ...
  // CHECK-DAG: %[[RANK:.+]] = arith.constant -1
  // CHECK-DAG: %[[COUNT:.+]] = arith.constant -1
  // CHECK: %[[CHANNEL:.+]] = hal.channel.create device(%[[DEVICE]] : !hal.device) affinity(%[[AFFINITY]]) rank(%[[RANK]]) count(%[[COUNT]]) : !hal.channel
