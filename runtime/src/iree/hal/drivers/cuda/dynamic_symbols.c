...
static iree_status_t iree_hal_nccl_dynamic_symbols_resolve_all(
    iree_hal_nccl_dynamic_symbols_t* syms) {
#define NCCL_PFN_DECL(ncclSymbolName, ...)                            \
  {                                                                   \
    static const char* kName = #ncclSymbolName;                       \
    IREE_RETURN_IF_ERROR(iree_dynamic_library_lookup_symbol(          \
        syms->loader_library, kName, (void**)&syms->ncclSymbolName)); \
  }
...
#include "iree/hal/drivers/cuda/dynamic_symbol_tables_nccl.h"  // IWYU pragma: keep
#undef NCCL_PFN_DECL
...
iree_status_t iree_hal_nccl_dynamic_symbols_initialize(
    iree_allocator_t host_allocator,
    iree_hal_nccl_dynamic_symbols_t* out_syms) {
  IREE_TRACE_ZONE_BEGIN(z0);
  memset(out_syms, 0, sizeof(*out_syms));
  iree_status_t status = iree_dynamic_library_load_from_files(
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_nccl_dynamic_symbols_resolve_all(out_syms);
  }
  if (!iree_status_is_ok(status)) {
    iree_hal_nccl_dynamic_symbols_deinitialize(out_syms);
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}
